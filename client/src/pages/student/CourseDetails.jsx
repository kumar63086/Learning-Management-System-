...
const CourseDetails = () => {
  ...
  const { allCoures, caluculate, calculaterChaptersTime, calculaterCourseDuration, calculateNoOfLectures, currency } = useContext(AppContext)
...
useEffect(() => {
  if (allCoures) {
    const foundCourse = allCoures.find(course => course._id === id);
...
if (!courseData) {
  ...
  const rating = Math.floor(caluculate(courseData))
    ...
    <>
      <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 pt-10 text-left">
        ...
        <div className="max-w-xl z-10 text-gray-500">
          ...
          <p className="text-sm">
            Course by <span className="text-blue-600 underline">Kumar</span>
          </p>
                    ...
                    <ul className="list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300">
                      {chapter.chapterContent.map((lecture, i) => (
                        <li key={i} className="flex items-start gap-2 py-1">
                          <img src={assets.play_icon} alt="lecture-thumbnail" className="w-4 h-4 mt-1" />
  ...
  <div className="relative">
    ...
    <button onClick={() => setPlayerData(null)} className="absolute top-2 right-2 bg-white rounded-full shadow p-1">✕</button>
  </div>
) : (
  <img src={courseData.courseThumbnail} alt="course-thumbnail" className='w-full' />
         ...
         <div className='p-5'>
         <div className='flex items-center gap-2'>
          <img src={assets.time_left_clock_icon} alt="time-left-clock-icon" className='w-4 h-4 mt-1' />
         ...
         <div className='flex items-center text-sm md:text-default gap-4 pt-2 mb:pt-4 text-gray-500'>
          <div className='flex items-center gap-1'>
            <img src={assets.star}  alt='star' className='w-3.5 h-3.5' />
            <p>{caluculate(courseData)}</p>
          </div>
          <div className='h-4 w-px bg-gray-500/40'></div>
          <div className='flex items-center gap-1'>
            <img src={assets.time_clock_icon}  alt='clock icon' className='w-3.5 h-3.5' />
            <p>{calculaterCourseDuration(courseData)}</p>
          </div>
          <div className='h-4 w-px bg-gray-500/40'></div>
          <div className='flex items-center gap-1'>
            <img src={assets.lesson_icon}  alt='clock icon' className='w-3.5 h-3.5' />
            <p>{calculateNoOfLectures(courseData)} lessons</p>
          </div>
         </div>
         <button  class="md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 text-white font-medium">{isAlreadyAdded?'Already Enrolled':'Enroll Now'}</button>
         <div className='pt-6'>
